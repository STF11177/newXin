...
#import "gradeDatePickView.h"

@interface gradeDatePickView()
{
    NSString *_dateStr;
}
...
@end
@implementation gradeDatePickView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(hidePicker)];
...
- (void)configuration {
    ...
    dateBgView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    ...
    [commitBtn addTarget:self action:@selector(surePickerView:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)surePickerView:(UIButton *)button{
    
    DDLog(@"%@",self.fromWhere);
    //确定
    if (button.tag == 1) {
        //确定
        self.completeBlock(_dateStr,self.fromWhere);
    }
