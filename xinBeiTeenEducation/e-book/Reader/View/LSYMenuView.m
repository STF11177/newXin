...
-(sliderView *)prograssView{
    
    if (!_prograssView) {
        
        _prograssView = [[sliderView alloc]init];
    }
...
-(void)menuViewInvokeTopCatalog:(LSYTopMenuView *)topMenu{
    
    if ([self.delegate respondsToSelector:@selector(menuViewInvokeTopCatalog:)]) {
        [self.delegate menuViewInvokeTopCatalog:topMenu];
    }
}

-(void)menuViewSetting:(LSYTopMenuView *)topMenu{
