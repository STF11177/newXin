...
@interface englishViewController ()<UITableViewDelegate,UITableViewDataSource,textualCellDelegate,backViewdelegate>
{
...
@property (nonatomic,assign) BOOL pullNew;//刷新
@property (nonatomic,assign) BOOL biaoJi;
@property (nonatomic,assign) BOOL netStatus1;
@property (nonatomic,strong) backGroundView *groundView;
...
- (void)viewDidLoad {
    ...
    NSDictionary *param = @{@"type":@"1",@"CreateDate":@"",@"userId":userId,@"token":tokenStr};
...
-(UITableView *)tableView{
    ...
    if (!_tableView) {
        ...
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake( 0, 64 +40, SCREEN_WIDTH, SCREEN_HEIGHT - 64 - 40) style:UITableViewStyleGrouped];
...
//        self.netStatus1 = NO;
        if (self.dataArray.count == 0) {
            ...
            self.groundView  = [[backGroundView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
            self.groundView.delegate = self;
...
-(void)loadMore{
    ...
    self.tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        ...
        NSDictionary *param = @{@"type":@"1",@"CreateDate":dateStr,@"userId":userId,@"token":tokenStr};
        weakSelf.pullNew = NO;
        [weakSelf loadAllDataWithParam:param];
